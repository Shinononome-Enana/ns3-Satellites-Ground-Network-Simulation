...
int main() {
    ...
    for(k=0; k<=TOTAL_SIMULATION_TIME; k+=DELTA_T)
    {
        ...
        for(i=0; i<=NETWORK_ROWS-1; i++)
        {
            for(j=0; j<=NETWORK_COLS-1; j++)
            {
                ...
                double distance = std::sqrt(std::pow(SAT_X[i][j]-CHINA_BASE_STATION_X,2)+
                                            ...
                                            std::pow(HEIGHT,2));
                if(distance > SEEABLE)  continue;

                if(service_time[i][j] > max_service_time)
                {
                    max_service_time = service_time[i][j];
                    con_number = i*NETWORK_ROWS + j;
                }
            }
        }
        CHINA_connected_num[fll] = con_number;
        max_service_time = 0;
        ...
        USA_connected_num[fll] = con_number;
        max_service_time = 0;
        fll++;
    }
    ...
    std::ofstream chinaFile("LRST_sat1.txt");
    if (chinaFile.is_open()) {
        for (int i = 0; i < CHINA_size; ++i) {
            chinaFile << CHINA_connected_num[i] << "\n";
        }
        chinaFile.close();
        std::cout << "写入文件成功：LRST_sat1.txt" << std::endl;
    } else {
        std::cerr << "无法打开文件：LRST_sat1.txt" << std::endl;
    }

    int USA_size = sizeof(USA_connected_num) / sizeof(USA_connected_num[0]);
    std::ofstream usaFile("LRST_sat2.txt");
    if (usaFile.is_open()) {
        for (int i = 0; i < USA_size; ++i) {
            usaFile << USA_connected_num[i] << "\n";
        }
        usaFile.close();
        std::cout << "写入文件成功：LRST_sat2.txt" << std::endl;
    } else {
        std::cerr << "无法打开文件：LRST_sat2.txt" << std::endl;
    }
